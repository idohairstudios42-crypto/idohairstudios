...
function PaymentConfirmation({ formData, selectedService, onConfirm, onCancel }: PaymentConfirmationProps) {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-70">
      <div className="bg-black p-6 rounded-lg border border-white/20 shadow-xl max-w-md w-full mx-4">
        ...
        <div className="space-y-4 mb-6">
          <div className="bg-black/60 border border-white/10 p-4 rounded-md">
            <h4 className="text-white/80 font-medium mb-2">Appointment Summary</h4>
            <div className="space-y-2 text-sm">
              <div className="flex items-start">
                <Calendar className="text-white/60 mr-2 h-4 w-4 mt-0.5" />
                <div>
              ...
              <div className="flex items-start">
                <Clock className="text-white/60 mr-2 h-4 w-4 mt-0.5" />
                <div>
              ...
              <div className="flex items-start">
                <User className="text-white/60 mr-2 h-4 w-4 mt-0.5" />
                <div>
              ...
              <div className="flex items-start">
                <Phone className="text-white/60 mr-2 h-4 w-4 mt-0.5" />
                <div>
              ...
              <div className="flex items-start">
                <Scissors className="text-white/60 mr-2 h-4 w-4 mt-0.5" />
                <div>
                  <p className="text-white">{selectedService.name}</p>
                  <p className="text-white/60 text-xs">{selectedService.description}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-black/60 border border-white/10 p-4 rounded-md">
            <h4 className="text-white/80 font-medium mb-2">Payment Details</h4>
        ...
        <div className="space-y-3">
          <button
            onClick={onConfirm}
            className="w-full py-3 px-4 rounded-md text-black font-medium bg-white hover:bg-white/90 transition flex items-center justify-center border border-white/30"
          >
          ...
          <button
            onClick={onCancel}
            className="w-full py-2 px-4 rounded-md text-white/80 font-medium border border-white/20 hover:bg-white/10 transition"
          >
            Cancel
          </button>
        </div>
...
function SuccessModal({ isOpen, onClose, paymentRequired, appointmentStatus }: SuccessModalProps) {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-70">
      <div className="bg-black p-8 rounded-lg border border-white/20 shadow-xl max-w-md w-full mx-4 transform transition-all duration-300 ease-in-out scale-100">
        <div className="text-center">
          <div className="mx-auto flex items-center justify-center h-12 w-12 rounded-full bg-white/10 mb-4 border border-white/20">
          ...
          <div className="mt-5">
            <button
              type="button"
              className="w-full inline-flex justify-center py-2 px-4 border border-white/30 shadow-sm text-sm font-medium rounded-md text-black bg-white hover:bg-white/90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-white/50"
              onClick={onClose}
            >
              Return to Home
            </button>
...
export default function AppointmentForm() {
  ...
  const renderDayContents = useCallback((day: number, date: Date) => {
    ...
    return (
      <div className="relative flex items-center justify-center">
        <span>{day}</span>
        {isAvailable && (
          <div className="absolute -top-0.5 -right-0.5 text-white text-xs">
            🌸
          </div>
        )}
      </div>
      ...
      const checkInterval = setInterval(() => {
        ...
        .then(verifyData => {
          ...
          if (verifyData.success) {
            ...
            // Only show error if payment actually failed (not pending)
            if (verifyData.data && verifyData.data.status === 'failed') {
              console.error('Payment failed:', verifyData);
              clearInterval(checkInterval);
              setIsProcessingPayment(false);
              setError('Payment was not successful. Please try again or contact support.');
            } else {
              // Still processing, don't show error
              console.log('Payment verification in progress:', verifyData);
            }
          }
        })
        .catch(error => {
  ...
  if (loading) {
    return (
      <div className="py-10 px-5 text-center">
        <h2 className="font-['Noto_Serif_Display'] text-3xl mb-6 text-white">Loading Appointment Form...</h2>
        <p className="text-white/70">Please wait while we load the available services and dates.</p>
      </div>
    )
  }
  ...
  return (
    <div className="py-10 px-5" style={{ fontFamily: "'Bebas Neue', sans-serif" }}>
      <h2 className="font-['Noto_Serif_Display'] text-3xl mb-6 text-center text-white">Appointment Details</h2>
      ...
      <div className="w-full flex flex-col items-center justify-center mb-6 animate-bounce-slow">
        <p className="text-white/70 text-sm mb-2">Scroll down to book your appointment</p>
        <svg 
      ...
      {noSlotsAvailable ? (
        <div className="text-center py-8 bg-black rounded-md border border-white/20">
          <h3 className="text-lg font-medium mb-2 text-white">No Available Slots</h3>
          <p className="text-white/70 text-sm mb-4">
            There are currently no available appointment slots. Please check back later or contact us directly.
          </p>
          <div className="flex justify-center space-x-4">
            <a 
              href="tel:+233548947612" 
              className="px-4 py-2 bg-white/10 border border-white/20 rounded text-sm hover:bg-white/20 transition-colors duration-300 text-white"
            >
              Call Us
            </a>
            <a 
              href="https://wa.me/233548947612" 
              className="px-4 py-2 bg-white text-black border border-white/30 rounded text-sm hover:bg-white/90 transition-colors duration-300"
            >
              WhatsApp
            </a>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 bg-black/80 backdrop-blur-sm p-6 rounded-lg border border-white/20 shadow-lg">
          {/* Name field */}
          <div>
            <label htmlFor="name" className="block text-sm font-['Noto_Serif_Display'] text-white mb-1">
              Your Full Name
            </label>
            <input
              type="text"
              ...
              className="w-full py-2.5 px-4 bg-black border border-white/20 rounded-md text-white placeholder:text-white/50 focus:outline-none focus:border-white focus:ring-1 focus:ring-white/50 transition-colors"
          ...
          <div>
            <label htmlFor="phone" className="block text-sm font-['Noto_Serif_Display'] text-white mb-1">
              Phone Number
            </label>
            <input
              type="tel"
              ...
              onChange={handleInputChange}
              className={`w-full py-2.5 px-4 bg-black border ${phoneError ? 'border-white' : 'border-white/20'} rounded-md text-white placeholder:text-white/50 focus:outline-none focus:border-white focus:ring-1 focus:ring-white/50 transition-colors`}
              placeholder="Enter your phone number"
            />
            {phoneError && <p className="text-white text-xs mt-1">{phoneError}</p>}
          </div>
          
          {/* Social Media fields */}
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label htmlFor="snapchat" className="block text-sm font-['Noto_Serif_Display'] text-white mb-1">
                Snapchat (Optional)
              </label>
              <input
                type="text"
            ...
            <div>
              <label htmlFor="whatsapp" className="block text-sm font-['Noto_Serif_Display'] text-white mb-1">
                WhatsApp Number
              </label>
              <input
                type="text"
          ...
          <div>
            <label htmlFor="serviceCategory" className="block text-sm font-['Noto_Serif_Display'] text-white mb-1">
              Service Category
            </label>
            <select
              id="serviceCategory"
              ...
              onChange={handleInputChange}
              className="w-full py-2.5 px-4 bg-black border border-white/20 rounded-md text-white appearance-none focus:outline-none focus:border-white focus:ring-1 focus:ring-white/50 transition-colors"
            >
          ...
          <div>
            <label htmlFor="service" className="block text-sm font-['Noto_Serif_Display'] text-white mb-1">
              Service Type
            </label>
            <select
              id="service"
              ...
              className="w-full py-2.5 px-4 bg-black border border-white/20 rounded-md text-white appearance-none focus:outline-none focus:border-white focus:ring-1 focus:ring-white/50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
          ...
          <div>
            <label htmlFor="preferredLength" className="block text-sm font-['Noto_Serif_Display'] text-white mb-1">
              Preferred Length
            </label>
            <select
              id="preferredLength"
          ...
          <div>
            <label htmlFor="hairColor" className="block text-sm font-['Noto_Serif_Display'] text-white mb-1">
              Hair Color
            </label>
            <input
              type="text"
          ...
          <div className="relative">
            <label className="block text-sm font-['Noto_Serif_Display'] text-white mb-1">
              Appointment Date
            </label>
            <DatePicker
              ...
              dayClassName={(date: Date) => {
                return isDateAvailable(date)
                  ? "!text-white !bg-white/20 !rounded-md !font-bold ring-1 ring-white/40"
            ...
            />
            <p className="mt-2 text-xs text-white/60">Highlighted dates are available.</p>
          </div>
          ...
          <button
            ...
            className="w-full py-3 bg-white text-black hover:bg-white/90 rounded-md font-medium transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center border border-white/30"
          >
            {isProcessingPayment ? (
              <>
